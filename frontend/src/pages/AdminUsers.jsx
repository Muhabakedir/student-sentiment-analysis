...
export default function AdminUsers() {
  ...
  if (!isSuperAdmin) {
    return (
      <div className="bg-gradient-to-br from-violet-900/40 to-indigo-900/30 dark:from-violet-900/40 dark:to-indigo-900/30 rounded-2xl p-10 text-center border border-violet-500/20 dark:border-violet-500/20 shadow-lg backdrop-blur-xl">
        <Crown size={32} className="mx-auto text-amber-400 mb-3" />
        <p className="text-sm font-semibold text-slate-200 dark:text-slate-200">Superadmin Access Required</p>
        <p className="text-xs text-slate-400 dark:text-slate-400 mt-1">Only the superadmin can manage administrator accounts.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {resetRequests.length > 0 && (
        <div className="bg-amber-500/15 dark:bg-amber-500/15 border border-amber-500/30 dark:border-amber-500/30 rounded-2xl px-5 py-4 backdrop-blur-xl">
          <div className="flex items-start gap-3">
            <AlertCircle size={18} className="text-amber-400 dark:text-amber-400 shrink-0 mt-0.5" />
            <div>
              <p className="text-sm font-semibold text-amber-300 dark:text-amber-300">
                ...
                {resetRequests.map((r, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center justify-between gap-4">
                      <p className="text-xs text-amber-400 dark:text-amber-400">
                      ...
                      {user && (
                        <button
                          ...
                          className="text-xs px-3 py-1 rounded-lg bg-amber-500/40 hover:bg-amber-500/60 text-amber-200 font-medium transition-colors shrink-0 border border-amber-500/30"
                        >
                          Send Reset Link
                        </button>
                      )}
      ...
      {/* Add Admin Form */}
      <div className="bg-gradient-to-br from-violet-900/40 to-indigo-900/30 dark:from-violet-900/40 dark:to-indigo-900/30 rounded-2xl p-6 shadow-lg border border-violet-500/20 dark:border-violet-500/20 backdrop-blur-xl">
        <h2 className="text-sm font-semibold text-slate-200 dark:text-slate-200 mb-5 flex items-center gap-2">
        ...
        <form onSubmit={handleAdd} className="space-y-4">
          <div className="flex flex-wrap gap-3">
            <div className="flex-1 min-w-48">
              <label className="block text-xs font-medium text-slate-400 dark:text-slate-400 mb-1.5">Email</label>
          ...
          <label className="flex items-center gap-3 cursor-pointer w-fit">
            ...
            <span className="text-sm text-slate-200 dark:text-slate-200 flex items-center gap-1.5">
        ...
        {error   && <p className="mt-3 text-xs text-rose-400 bg-rose-500/15 dark:bg-rose-500/15 border border-rose-500/30 dark:border-rose-500/30 rounded-xl px-3 py-2">{error}</p>}
        {success && <p className="mt-3 text-xs text-emerald-400 bg-emerald-500/15 dark:bg-emerald-500/15 border border-emerald-500/30 dark:border-emerald-500/30 rounded-xl px-3 py-2">{success}</p>}
      </div>
      ...
      <div className="bg-gradient-to-br from-violet-900/40 to-indigo-900/30 dark:from-violet-900/40 dark:to-indigo-900/30 rounded-2xl shadow-lg border border-violet-500/20 dark:border-violet-500/20 backdrop-blur-xl">
        <div className="px-5 py-4 border-b border-violet-500/20 dark:border-violet-500/20 flex items-center justify-between">
          <h2 className="text-sm font-semibold text-slate-200 dark:text-slate-200 flex items-center gap-2">
            <Shield size={15} className="text-violet-400" />
            All Admin Accounts
          </h2>
          <span className="text-xs text-slate-400 dark:text-slate-400 bg-violet-500/20 dark:bg-violet-500/20 px-2.5 py-1 rounded-lg">
            {users.length} total
          </span>
        </div>

        {loading ? (
          <div className="p-8 text-center text-sm text-slate-400 dark:text-slate-400">Loading...</div>
        ) : (
          <div className="divide-y divide-violet-500/10 dark:divide-violet-500/10">
            {users.map(user => (
              <div key={user.id} className="px-5 py-4 flex items-center justify-between gap-4 hover:bg-violet-500/10 dark:hover:bg-violet-500/10 transition-colors">
                <div className="flex items-center gap-3 min-w-0">
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold shrink-0
                    ${user.is_superadmin ? "bg-amber-500/20 dark:bg-amber-500/20 text-amber-400 dark:text-amber-400" : "bg-violet-500/20 dark:bg-violet-500/20 text-violet-400 dark:text-violet-400"}`}>
                    {user.is_superadmin ? <Crown size={15} /> : user.email.slice(0, 2).toUpperCase()}
                  </div>
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-sm font-medium text-slate-200 dark:text-slate-200 truncate">{user.email}</p>
                      {user.is_superadmin && (
                        <span className="text-xs bg-amber-500/20 dark:bg-amber-500/20 text-amber-400 dark:text-amber-400 px-2 py-0.5 rounded-full font-medium shrink-0">
                          Superadmin
                        </span>
                      )}
                      {user.status === "pending" && (
                      ...
                      {hasPendingReset(user.email) && (
                        <span className="text-xs bg-amber-500/20 dark:bg-amber-500/20 text-amber-400 dark:text-amber-400 px-2 py-0.5 rounded-full font-medium shrink-0 flex items-center gap-1">
                          <AlertCircle size={10} /> Reset requested
                        </span>
                      )}
                    </div>
                    <p className="text-xs text-slate-500 dark:text-slate-500">Added {new Date(user.created_at).toLocaleDateString()}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${
                    user.is_active
                      ...
                      : "bg-slate-500/20 dark:bg-slate-500/20 text-slate-400 dark:text-slate-400"
                  }`}>
                  ...
                  <button
                    onClick={() => { setSetPassModal({ id: user.id, email: user.email }); }}
                    className="p-2 rounded-xl text-slate-400 hover:text-violet-400 hover:bg-violet-500/20 dark:hover:bg-violet-500/20 transition-colors"
      ...
      {setPassModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-gradient-to-br from-violet-900/60 to-indigo-900/50 dark:from-violet-900/60 dark:to-indigo-900/50 rounded-2xl shadow-2xl border border-violet-500/30 dark:border-violet-500/30 p-6 w-full max-w-md backdrop-blur-xl">
            <div className="flex items-center justify-between mb-5">
              <div>
                <h3 className="text-sm font-semibold text-slate-200 dark:text-slate-200 flex items-center gap-2">
                  <KeyRound size={15} className="text-violet-400" />
                  Trigger Password Reset
                </h3>
                <p className="text-xs text-slate-400 dark:text-slate-400 mt-0.5">{setPassModal.email}</p>
              </div>
            ...
            <div className="space-y-4">
              <div className="bg-violet-500/15 dark:bg-violet-500/15 border border-violet-500/30 dark:border-violet-500/30 rounded-xl p-4">
                <p className="text-sm text-violet-300 dark:text-violet-300 font-medium">
                  Send password reset link?
                </p>
                <p className="text-xs text-violet-400 dark:text-violet-400 mt-1">
              ...
              <div className="flex gap-3">
                <button onClick={() => setSetPassModal(null)}
                  className="flex-1 py-2.5 rounded-xl border border-violet-500/30 dark:border-violet-500/30 text-slate-300 dark:text-slate-300 text-sm font-medium hover:bg-violet-500/20 dark:hover:bg-violet-500/20 transition-colors">
                  Cancel
                </button>
