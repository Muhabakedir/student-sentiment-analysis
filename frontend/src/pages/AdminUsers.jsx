  ...
  if (!isSuperAdmin) {
    return (
      <div className="bg-white dark:bg-gray-900 rounded-2xl p-10 text-center border border-gray-100 dark:border-gray-800 shadow-sm">
        <Crown size={32} className="mx-auto text-yellow-400 mb-3" />
        <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">Superadmin Access Required</p>
        <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Only the superadmin can manage administrator accounts.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
                    ...
                    <div key={i} className="flex items-center justify-between gap-4">
                      ...
                      {user && (
                        <button
                          ...
                          className="text-xs px-3 py-1 rounded-lg bg-amber-600 hover:bg-amber-700 text-white font-medium transition-colors shrink-0"
                        >
                          Send Reset Link
                        </button>
                      )}
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-800">
        ...
        {error   && <p className="mt-3 text-xs text-red-600 bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800 rounded-xl px-3 py-2">{error}</p>}
        {success && <p className="mt-3 text-xs text-green-700 bg-green-50 dark:bg-green-900/20 border border-green-100 dark:border-green-800 rounded-xl px-3 py-2">{success}</p>}
      </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800">
        <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
          ...
          <span className="text-xs text-gray-400 dark:text-gray-500 bg-gray-100 dark:bg-gray-800 px-2.5 py-1 rounded-lg">
            {users.length} total
          </span>
        </div>
        ...
        {loading ? (
          ...
          <div className="divide-y divide-gray-50 dark:divide-gray-800">
            {users.map(user => (
              <div key={user.id} className="px-5 py-4 flex items-center justify-between gap-4 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                <div className="flex items-center gap-3 min-w-0">
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold shrink-0
                    ${user.is_superadmin ? "bg-yellow-100 dark:bg-yellow-900/40 text-yellow-700 dark:text-yellow-400" : "bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400"}`}>
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {hasPendingReset(user.email) && (
                        <span className="text-xs bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 px-2 py-0.5 rounded-full font-medium shrink-0 flex items-center gap-1">
                          <AlertCircle size={10} /> Reset requested
                        </span>
                      )}
                    </div>
                    <p className="text-xs text-gray-400 dark:text-gray-500">Added {new Date(user.created_at).toLocaleDateString()}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button onClick={() => handleToggle(user.id, user.email)}
                    ...
                    {user.is_active ? <ToggleRight size={18} className="text-green-500" /> : <ToggleLeft size={18} />}
                  </button>
      ...
      {setPassModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-800 p-6 w-full max-w-md">
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{setPassModal.email}</p>
              </div>
