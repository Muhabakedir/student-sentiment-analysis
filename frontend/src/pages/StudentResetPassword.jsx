...
import { GraduationCap, Eye, EyeOff, Sun, Moon, CheckCircle } from "lucide-react";
...
export default function StudentResetPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API}/api/auth/student-reset-confirm`, {
        method: "POST",
            ...
            {success ? (
              <div className="space-y-4">
                <div className="flex items-center gap-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-2xl p-4">
                  ...
                  <div>
                    <p className="text-green-600 dark:text-green-400 font-medium">Password reset successful!</p>
                    <p className="text-green-500 dark:text-green-400/80 text-sm">You can now log in with your new password.</p>
                  </div>
                </div>
                <button
